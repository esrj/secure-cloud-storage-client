...
export const Validators = {
  ...
  password(input) {
    if (!input) return { valid: false, message: 'Password is required.' }
    if (input.length < 8 || input.length > 32) {
      return { valid: false, message: 'Password must be between 8 and 32 characters long.' }
    }
    if (!/[A-Z]/.test(input)) return { valid: false, message: 'Password must contain at least one uppercase letter.' }
    if (!/[a-z]/.test(input)) return { valid: false, message: 'Password must contain at least one lowercase letter.' }
    if (!/[0-9]/.test(input)) return { valid: false, message: 'Password must contain at least one number.' }
    if (!/[!@#$%^&*(),.?":{}|<>_\-+=\[\]\\\/;`~]/.test(input))
  ...
  uuidv4(input) {
    ...
    if (!pattern.test(input)) return { valid: false, message: 'Invalid UUIDv4 format.' }
    return { valid: true }
  },

  folderName(input) {
    const pattern = /^(?!.*[\\/:*?"<>|])[\p{L}\p{N}\p{M}\s_-]{1,255}$/u
    if (!input) return { valid: false, message: 'Folder name is required.' }
    if (!pattern.test(input)) {
      return {
        valid: false,
